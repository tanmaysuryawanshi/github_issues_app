...
class IssueInfoViewModel @Inject constructor (
    private val getIssueInfo: GetIssueInfo
): ViewModel() {
private val _searchQuery=mutableStateOf("")
    val searchQuery: State<String> = _searchQuery

    private val _state= mutableStateOf(IssueInfoState())
    val state:State<IssueInfoState> =_state
                    ...
                    when(result){
                        is Resource.Success->{
                          _state.value=state.value.copy(
                              issueInfoItems = result.data?: emptyList(),
                              isLoading = false
                          )}
                            is Resource.Error ->{
                                _state.value=state.value.copy(
